...
exports.getValue = async function (selector, frameId) {
  ...
  const value = await this.getNodeValue(node)
 ...
 * @param {NodeObject} node - The Node Object
 * @param {string} [value] - The value to set the node to
 */
...
exports.querySelector = async function (selector, frameId) {
  ...
  try {
    ...
    throw new Error(`Can not query selector ${selector} in frame "${frameId || 'root'}"`, err)
  }
}
...
exports.select = async function (selector, value, frameId) {
  ...
  if (typeof value !== 'undefined') {
    ...
    await this.getValue(selector, value, frameId)
  }
}
...
exports.saveScreenshot = async function (fileName = `screenshot-${Date.now()}`, captureOptions) {
  ...
  await writeFile(`${fileName}.${captureOptions.format || 'png'}`, await this.getScreenshot(captureOptions), 'base64')

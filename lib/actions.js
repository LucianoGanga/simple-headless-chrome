...
exports.injectRemoteScript = async function injectRemoteScript (src) {
  ...
  browserIsInitialized.call(this)
  const result = await this.evaluate(function (src) {
    const script = document.createElement('script')
    script.src = src
    document.head.appendChild(script)
  }, src)
...
exports.setCookie = async function (url, name, value) {
  ...
  return this.client.Network.setCookie({url: url, name: name, value: value})
}
...
exports.resizeFullScreen = async function () {
  ...
  // await Emulation.forceViewport({x: 0, y: 0, scale: 1}) // Deprecated by Chrome DevTools Protocol
}

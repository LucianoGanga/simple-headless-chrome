 ...
 * @return {object} - Request status and data
 */
exports.ajax = async function (url, action = 'GET', data = {}, options = {}) {
  ...
  return this.evaluateAsync(function (url, action, data, options) {
    return new Promise((resolve, reject) => {
      const xhr = new XMLHttpRequest()
      xhr.open(action, url)
      xhr.setRequestHeader('Content-Type', options.contentType)
      ...
      xhr.onerror = function (e) {
        ...
        const error = new Error(`Error while making request ${action} to URL "${url}"`)
        error.requestData = data
        error.options = options
        error.response = {
          responseText: xhr.responseText,

 ...
 * Start screencast
 */
exports.startScreencast = async function () {
 ...
 * Stop screencast
 */
exports.stopScreencast = async function () {
 ...
 * Start screencast
 */
exports.saveScreencast = async function (fileName) {
  debug(`:: saveScreencast => Save screencast...`)
  browserIsInitialized.call(this)

  this.client.Page.screencastFrame(async (frame) => {
    const frameData = frame.data
    const screenshot = Buffer.from(frameData, 'base64')
    const metadata = frame.metadata
    const frameNumber = frame.sessionId
    console.log(frameNumber, metadata)
    await fs.writeFile(`${fileName}-${frameNumber}-${metadata.timestamp}.png`, screenshot)
  })
...
exports.saveScreenshot = async function (fileName = `screenshot-${Date.now()}`, { format = 'png', quality, clip, fromSurface, selector, fullPage } = {}) {
  ...
  await fs.writeFile(`${fileName}.${format}`, await this.getScreenshot({
    format,
    quality,
    clip,
    fromSurface,

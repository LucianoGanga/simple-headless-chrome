...
class HeadlessChrome {
  ...
  async init ({ privateTab = false } = {}) {
    ...
    try {
      ...
      // Support default tab within the main class to support all actions without having to create a first new tab
  ...
  async newTab ({ privateTab = false } = {}) {
     ...
     */
    if (Object.keys(this.tabs).length >= this.options.chrome.maxTabs) {
      debug(`>> newTab => Can't open a new private tab without headless mode.`)
      return
    }
    ...
    if (privateTab) {
      browserContextId = await this.browser.Target.createBrowserContext()
    }
    ...
    try {
      ...
      const { targetId } = await Target.createTarget(params).targetId
      ...
      if (privateTab) {
        tab.privateTab = browserContextId
      }

      debug('>> newTab => Created new tab, and going to attach CDP')
      return tab
    } catch (err) {
      debug(`>> newTab => Could not create a new target. Error code: ${err.code}`, err)
...
class Tab extends EventEmitter {
  ...
  async init () {
    this.client = await _attachCdpToTab(this.host, this.port, this.tabId)
   ...
   */
  getBrowserContextId () {
    if (typeof this.privateTab !== 'undefined') {
...
async function _attachCdpToChrome (host = 'localhost', port = 9222, remote = false) {
  ...
  try {
    ...
    browser = await CDP({host, port, remote})

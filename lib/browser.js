  ...
  async init () {
    if (this.options.launchChrome) {
      ...
      this.chromeInstance = await _launchChrome({
        ...
        launchAttempts: this.options.launchAttempts
      })
...
async function _launchChrome ({ headless = true, port = 0, launchAttempts }) {
  ...
  while (!instance && attempt++ < launchAttempts) {
    try {
      instance = await chromeInstance.launch(chromeOptions)
    } catch (err) {
      if (err.code === 'EAGAIN') {
  ...
  if (!instance) {
    throw new Error(`Can't not launch Chrome! (attempts: ${attempt}/${launchAttempts})`)
  }

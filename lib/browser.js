  ...
  async init () {
    if (this.options.launchChrome) {
      ...
      this.chromeInstance = await _launchChrome({
        ...
        disablePopupBlocking: this.options.chrome.disablePopupBlocking,
 ...
 */
async function _launchChrome ({ headless = true, disableGPU = true, noSandbox = false, disablePopupBlocking = false, port = 0, userDataDir, launchAttempts, handleSIGINT, chromeFlags }) {
  ...
  if(disablePopupBlocking) { chromeOptions.chromeFlags.push('--disable-popup-blocking') }

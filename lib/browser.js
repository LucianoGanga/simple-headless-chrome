...
class HeadlessChrome{
  ...
  async init({privateTab = false} = {}){
    ...
    try{
      ...
      this.tabs = []
      return (this.client = await this.newTab({privateTab}))
    } catch(err){
  ...
  async closeTab (tabId) {
    ...
    try {
      ...
      if (this.tabs[tabId].isPrivate())
        this.browser.Target.disposeBrowserContext(this.tabs[tabId].getBrowserContextId())
